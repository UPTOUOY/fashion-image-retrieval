...
Recall@1 0.49 → 0.88, mAP 0.20 → 0.62, top-5 실패는 168장에서 19장으로 줄었다. 다만 0.85를 넘어선 뒤로는(더 큰 백본, 데이터·학습량 확대) 0.874 → 0.884로 거의 안 움직였다. query 500개 기준 R@1 표준오차가 ~1.4%p라 이 정도 차이는 노이즈에 가깝고, 이 스케일에선 사실상 포화라고 본다.
...
FashionCLIP은 학습을 하나도 안 했는데(zero-shot 0.892) 내가 백본까지 튜닝한 DINOv2(0.882)와 사실상 같은 수준이었다. query가 500개라 R@1의 표준오차가 대략 1.4%p인데 둘 차이는 1%p라, "넘었다"기보다 **학습 없이 동률에 도달**했다고 보는 게 맞다. 그게 오히려 요점이다 — 튜닝에 며칠 쓴 것과 모델만 바꾼 게 비긴 거니까. 반면 FashionCLIP을 fine-tune하면 0.932로, 이건 노이즈(두 값 차이의 표준오차 약 2%p)를 넘는 실제 차이다. 결국 이 문제에선 백본을 얼마나 잘 튜닝하느냐보다 어떤 표현을 쓰느냐가 더 컸다.
...
fine-tune 전후로 patch 토큰이 CLS에 얼마나 정렬되는지 히트맵으로 그려봤다. 학습 전엔 배경이랑 사람 윤곽에 반응이 퍼져 있다가, 후엔 배경이 식고 옷 쪽으로 집중이 옮겨갔다. 위 실패 분석에서 말한 "장면을 본다 → 옷을 본다"가 숫자뿐 아니라 그림으로도 보였다.

![fine-tune 전/후 patch–CLS 집중도 히트맵 — 각 행 왼쪽부터 원본·학습 전·학습 후](results/attention_compare.png)

## 검색 쪽 숫자

임베딩 차원은 DINOv2-small 384 / DINOv2-base 768 / FashionCLIP 512. 인덱스는 FAISS `IndexFlatIP`인데, gallery가 8k 규모라 그냥 brute-force 전수 검색으로 충분했고 지연도 무시할 만하다. 실제 서비스처럼 gallery가 52k(In-Shop 전체) 이상으로 커지면 이 flat 인덱스는 안 맞고, IVF나 HNSW 같은 근사 최근접(ANN) 인덱스로 바꿔야 한다.
